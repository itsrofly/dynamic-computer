...
export async function createProject(Projects: Project[]): Promise<void> {
    ...
    // Create main.py file
    await ipcRenderer.invoke('writeFile', join(uniquePath, settings.file), "#Write the application code here\nprint('Hello, World!')");
...
export async function addMessage(Projects: Project[], index: number, role: "user" | "assistant", content: string): Promise<void> {
    ...
    await ipcRenderer.invoke('writeFile', join(project.path, 'settings.json'), JSON.stringify(projectSettings));
}
